...
extern xSemaphoreHandle       	xSemHand_json;
...
void webclient_datahandler(char *data, u16_t len) {
  ...
  xSemaphoreTake(xSemHand_json, portMAX_DELAY);
  if(len == 0) {
		...
		for(i=0; i<JSMN_NUM_TOKENS; ++i) {
			...
			if(curTok->type == JSMN_STRING) {
				...
				if(strcmp("queued_jobs", temp_buf) == 0) {
					...
					// HACK: check if the very first instruction is 0,0, if it is, nullify it
					/*
					if(plotter_instructions[0].x == 0 && plotter_instructions[0].y == 0) {
						plotter_instructions[0].type = NOT_INIT;
					}
					*/
				}
			}
